...
/**
 * Workflow persistent storage.
 *
 * Each workflow is stored individually under a per-ID key. This prevents
 * whole-library corruption from a single malformed record and makes individual
 * CRUD operations O(1) rather than O(n) full serialization.
 *
 * **Legacy migration:** If `workflows_json` exists, it is read once, split into
 * individual keys, and then the bulk key is deleted.
 *
 * **Format version** is stored under `workflow_store_version`. Bumping it
...
object WorkflowRepository {
    ...
    private const val KEY_LEGACY_WORKFLOWS = "workflows_json"
    ...
    private val idListLock = Any()

    // ── Initialization / migration ──

    private fun ensureMigrated(context: Context) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val version = prefs.getInt(KEY_STORE_VERSION, 0)
        if (version >= CURRENT_STORE_VERSION) return

        synchronized(lock) {
            if (prefs.getInt(KEY_STORE_VERSION, 0) >= CURRENT_STORE_VERSION) return // double-check

            // Migrate legacy bulk JSON → per-workflow keys
            val legacyRaw = prefs.getString(KEY_LEGACY_WORKFLOWS, null)
            if (!legacyRaw.isNullOrBlank()) {
                try {
                    val type = object : TypeToken<MutableList<HabitatWorkflow>>() {}.type
                    val list: List<HabitatWorkflow> = gson.fromJson(legacyRaw, type) ?: emptyList()
                    val editor = prefs.edit()
                    for (wf in list) {
                        editor.putString(KEY_WORKFLOW_PREFIX + wf.id, gson.toJson(wf))
                    }
                    ...
                    // Keep legacy data; don't overwrite with empty
                    Log.e(TAG, "Legacy migration failed — keeping bulk key", e)
                    try {
                        val backupKey = "workflows_json_backup_${System.currentTimeMillis()}"
                        prefs.edit().putString(backupKey, legacyRaw).commit()
                        Log.w(TAG, "Corrupt data backed up to $backupKey")
                    } catch (_: Exception) {}
    ...
    fun getAll(context: Context): List<HabitatWorkflow> {
        ...
        synchronized(lock) {
            ...
            val list = all.entries
                .filter { it.key.startsWith(KEY_WORKFLOW_PREFIX) }
                .mapNotNull { entry ->
                    try {
                        gson.fromJson(entry.value as? String ?: return@mapNotNull null, HabitatWorkflow::class.java)
                    } catch (e: Exception) {
                        Log.w(TAG, "Skipping corrupt workflow '${entry.key}'", e)
                        null
                    }
                }
    ...
    fun upsert(context: Context, workflow: HabitatWorkflow) {
        ...
        synchronized(lock) {
            prefs.edit()
                .putString(KEY_WORKFLOW_PREFIX + workflow.id, gson.toJson(workflow))
                .commit()
        }
    }
    ...
    fun delete(context: Context, id: String) {
        ...
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        synchronized(lock) {
            prefs.edit().remove(KEY_WORKFLOW_PREFIX + id).commit()
        }
    ...
    // ── Float-mounted workflow IDs ──

    fun getFloatMountedWorkflowIds(context: Context): Set<String> = synchronized(idListLock) {
        val raw = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            .getString(KEY_FLOAT_MOUNTED, null)
        raw?.split(",")?.filter { it.isNotBlank() }?.toSet() ?: emptySet()
    }

    fun addFloatMountedWorkflowId(context: Context, id: String) = synchronized(idListLock) {
        val ids = getFloatMountedWorkflowIds(context).toMutableSet()
        ids.add(id)
        saveMountedIds(context, ids)
    }

    fun removeFloatMountedWorkflowId(context: Context, id: String) = synchronized(idListLock) {
        val ids = getFloatMountedWorkflowIds(context).toMutableSet()
        ids.remove(id)
        saveMountedIds(context, ids)
    }
    ...
    private fun saveMountedIds(context: Context, ids: Set<String>) {
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            .edit().putString(KEY_FLOAT_MOUNTED, ids.joinToString(",")).commit()
    }

    // ── Internal helpers ──

    private fun saveAllInternalUnsafe(
        prefs: android.content.SharedPreferences,
        workflows: List<HabitatWorkflow>,
        keysToRemove: Set<String>? = null,
    ) {
        val editor = prefs.edit()
        for (wf in workflows) {
            try {
                ...
                Log.e(TAG, "Failed to serialize workflow '${wf.id}'", e)
            }
        }
    ...
    // ── Default workflows (shipped with the app) ──
