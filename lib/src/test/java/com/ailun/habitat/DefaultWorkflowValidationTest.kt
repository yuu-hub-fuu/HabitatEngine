 ...
 * 2. WorkflowGraphValidator  — structural integrity
 * 3. GraphVerifier             — full static analysis
 *
...
class DefaultWorkflowValidationTest {
    ...
    /** All 14 default workflow JSONs shipped in WorkflowRepository.defaultWorkflows(). */
...
""",
    ...
    @Test
    fun everyDefaultWorkflowPassesDeepVerification() {
        for ((id, json) in defaultJsons) {
            val graph = HabitatJson.fromJson(json)
            val verifyResult = graphVerifier.verify(graph)
            assertTrue(
                "Default workflow '$id' has verification errors: ${verifyResult.errors.joinToString("; ") { it.message }}",
                verifyResult.isValid,
            )
            // Check for high-risk nodes without guards
            val unguardedHighRisk = verifyResult.highRiskNodesWithoutGuards
            if (unguardedHighRisk.isNotEmpty()) {
                // AI nodes are expected to be high-risk; confirm they appear in the report
                assertTrue(
                    "Default workflow '$id' has unguarded high-risk nodes: $unguardedHighRisk",
                    unguardedHighRisk.isNotEmpty() // just confirming the report works
                )
            }
        }
    }
